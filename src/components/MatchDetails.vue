    ...
    <div class="container">
        ...
        <div class="scrollbar">
            ...
            <el-form>
                ...
                <el-form-item v-if="getChessClass(host.color)=='black'"  label="吃子">{{eat_chess}}</el-form-item>
                <el-form-item v-if="getChessClass(host.color)=='white'"  label="剩余棋子">{{play_out}}</el-form-item>
            </el-form>
            <el-divider/>
            <div class="role">
                {{challenger.name}}
            ...
            <el-form>
                ...
                <el-form-item v-if="getChessClass(challenger.color)=='black'"  label="吃子">{{eat_chess}}</el-form-item>
                <el-form-item v-if="getChessClass(challenger.color)=='white'"  label="剩余棋子">{{play_out}}</el-form-item>
            </el-form>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onReady() {
                setReady(this.roomId, this.$store.getters.player.id)
            },
            isPlayer() {
        ...
        computed: {
            ...
            eat_chess(){
                if(this.$store.getters.step.state){
                    return this.$store.getters.step.state.eat_c
                }else{
                    return 0
                }
            },
            play_out(){
                if(this.$store.getters.step.state){
                    if(this.$store.getters.step.state.play_out<=32){
                        return 16-Math.floor(this.$store.getters.step.state.play_out/2)
                    }else{
                        return 0
                    }
            }else{
